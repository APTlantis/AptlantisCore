...
```powershell
$drs = "D:\.library\aptlantis_core\DRS\drs.ps1"
  ...
  drs:
    ...
    steps:
      ...
      - name: Verify release manifest
        shell: pwsh
        run: D:\.library\aptlantis_core\DRS\drs.ps1 verify-manifest
      - name: Check release gate
        shell: pwsh
        run: D:\.library\aptlantis_core\DRS\drs.ps1 check-release
```
